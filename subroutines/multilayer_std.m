...
if N>2
	for ii=2:1:N-1
		...
		h_k1a1 = sbesselh(ALPHA,1,k1a1,'none');
		h_k1a0 = sbesselh(ALPHA,1,k1a0,'none');
		j_k1a1 = sbesselj(ALPHA,k1a1,'none');
		j_k1a0 = sbesselj(ALPHA,k1a0,'none');
...
%		end
		...
		D = 1-R_12*R_std(ii-1);
		R_std(ii) = R_21 + T_12*T_21*R_std(ii-1)/D;
		S(ii) = T_21/D;
	end
	T_std(1:N-2) = fliplr(cumprod( fliplr(S(2:N-1)) )).*S(1:N-2);
endif
